...
static void PostRequestMethod(std::string& content){
  ...
  if(strcmp(content_type,"application/x-www-form-urlencoded;charset=utf-8")!=0)
	  return;
  ...
  //r = net::core_get(0,content.c_str(),content.length(),
	  //          respone,flag,code);
  if (!respone.empty()&&r)
	printf("Content-type: application/octet-stream; charset=utf-8\r\n"
		  "\r\n"
		"%s",content_type);
}
...
int main(int agrc,char* argv[]){
	...
	std::string ipc_conn = "/var/www/tmp/bccorefile";
